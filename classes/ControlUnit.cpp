...
rtm::ControlUnit::ControlUnit(size_t type, int column, int row, LinesCounts linesCounts)
    : type_{ type }
    , column_{ column }
    , row_{ row }
...
rtm::ControlUnit::ControlUnit(size_t type, int column, int row, LinesCounts linesCounts, AngleType nullDirection)
    : type_{ type }
    , column_{ column }
    , row_{ row }
    , linesCounts_{ linesCounts }
...
void rtm::ControlUnit::ShowSprites(cocos2d::Layer* const layer)
{
    for (size_t i = 0; i < 4; ++i) {
        ...
        for (size_t l = 0; l < 3; ++l) {
            ...
            for (size_t k = 0; k < 5; ++k) {
                if (sprites_[i][l][k] != nullptr) {
                    layer->addChild(sprites_[i][l][k], zOrder);
...
void rtm::ControlUnit::ReleaseSprites(cocos2d::Layer* const layer)
{
    for (size_t i = 0; i < 4; ++i) {
        for (size_t l = 0; l < 3; ++l) {
            for (size_t k = 0; k < 5; ++k) {
                if (sprites_[i][l][k] != nullptr) {
                    layer->removeChild(sprites_[i][l][k]);
