...
rtm::DynamicObject* rtm::VehicleObject::CheckTurnArea_(WorldController * const world, bool isRight)
{
    float angleShift{ isRight ? TURN_VIEW_ANGLE_SHIFT : -TURN_VIEW_ANGLE_SHIFT };
...
rtm::DynamicObject* rtm::VehicleObject::CheckCrossroadArea_(WorldController* const world)
{
    float angleShift{ rotationAngle_ > 0 ? CROSSROAD_VIEW_ANGLE_SHIFT : -CROSSROAD_VIEW_ANGLE_SHIFT };
    return CheckForwardArea_(world, CROSSROAD_VIEW_RADIUS, CROSSROAD_VIEW_ANGLE, angleShift);
}
...
{
    rotationRadius_ = DistanceToNextCenter(GetX_(), GetY_(), GetAngle());
    return true;
}
...
bool rtm::VehicleObject::RotationTick_(WorldController* const world)
{
    ...
    if (rotationAngle_ == 0.f) {
        ...
        if (angle == Up || angle == Down) {
            SetX_(RoundToCenter(GetX_()));
        }
        else if (angle == Left || angle == Right) {
            SetY_(RoundToCenter(GetY_()));
        }
        else {
            SetX_(RoundCoordinate(GetX_()));
            SetY_(RoundCoordinate(GetY_()));
        }
